...
class App extends Component {
  ...
  makeAjaxCall(){
   ...
   async componentDidMount() {
    // this is a great place to make AJAX calls
    // Async Await Method
    const ajaxResponse = await this.makeAjaxCall();
    const difficulty = ajaxResponse.difficulty || 'Easy'
    this.setState({difficulty})

    // .This Method //

    // this.makeAjaxCall().then((response) => {
    //   this.setState({difficulty: response.difficulty});
    // })

    console.log('App: Component did run')
  }

  componentDidUpdate(prevProps, prevState) {
    if(prevState !== this.state.userId){
      
    }
    // make PUT requests to update database values
