...
function handleMarkerClick(evt) {
  ...
  const rowId = board[colId].indexOf(0);
...
function checkUp(colIdx, rowIdx) {
  ...
  const absVal = Math.abs(colArr[rowIdx] + colArr[rowIdx + 1] + colArr[rowIdx + 2] + colArr[rowIdx + 3]);
...
function checkRight(colIdx, rowIdx) {
  ...
  const absVal = Math.abs(board[colIdx][rowIdx] + board[colIdx + 1][rowIdx] + board[colIdx + 2][rowIdx] + board[colIdx + 3][rowIdx]);
...
function checkDiag(colIdx, rowIdx, vertOffset) {
  ...
  const absVal = Math.abs(board[colIdx][rowIdx] + board[colIdx + 1][rowIdx + vertOffset] + board[colIdx + 2][rowIdx + vertOffset * 2] + board[colIdx + 3][rowIdx + vertOffset * 3]);
  return absVal === 4 ? board[colIdx][rowIdx] : null;
}
...
function render() {
  ...
  board.forEach((colArray, colIdx) => {
    ...
    colArray.forEach((cell, rowIdx) => {
      ...
      const el = document.getElementById(`c${colIdx}r${rowIdx}`);
